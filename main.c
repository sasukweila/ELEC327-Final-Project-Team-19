...
void main(void)
{
    ...
    while(1){
        distance = diff/58;//calculate distance based on diff (difference between trigger and echo pulse) and 58 (speed of sound)
        dst_int = floor(distance);//round to integer value
        tmp_flt = distance - dst_int;//calculate floating point value (integer value minus float value)
        ltoa(dst_int, dst_char,10);//convert integer to string using base 10
        if (tmp_flt < 0.01) {
            dst_flt = floor(tmp_flt * 1000);
            ltoa(dst_flt,dst_flt_char,10);//convert integer to string using base 10
            //print characters to serial
        ...
        ser_output(newline);//print newline
    }
}
//Timer ISR
...
__interrupt void Timer_A(void){
        ...
        i += 1;
        TA1CCTL1 &= ~CCIFG ;
        if (i==2) {//check if both edges are captured
            diff=temp[i-1]-temp[i-2];//calculate time between pulses
            i=0;
        }
...
void ser_output(char *str){
    while(*str != 0) {//iterates as long as next char is not null
        while (!(IFG2&UCA0TXIFG));//check UART transmit buffer
        UCA0TXBUF = *str++;//write next char to buffer
        }
}
